...
async function adobeScraper(searchQuery, maxPages = 10) {
  const allJobs = [];
const baseUrl = 'https://careers.adobe.com/us/en/search-results';
  ...
  await browser.close();
  console.log(
    `\n🎉 ADOBE Scraping completed! Found ${allJobs.length} total jobs across ${lastProcessedPage} page${lastProcessedPage === 1 ? '' : 's'}.`
  );
  return allJobs;
}
