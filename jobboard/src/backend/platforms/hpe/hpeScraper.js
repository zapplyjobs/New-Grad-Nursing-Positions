...
const { filterUSJobs } = require('../../filterUSJobs');
...
async function hpeScraper(searchQuery, maxPages = 30) {
  ...
  const allJobs = [];
  const baseUrl = 'https://hpe.wd5.myworkdayjobs.com';
  
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      console.log(`Scraping page ${pageNum}...`);
      // Construct URL with search query
      const searchUrl = `${baseUrl}/WFMathpe?q=${encodeURIComponent(searchQuery)}`;
  ...
  console.log(`\n HPE Scraping completed! Found ${allJobs.length} total jobs.`);
  
  // Filter for US jobs only
  const usJobs = filterUSJobs(allJobs, {
    includeRemote: true,
    includeTerritories: true,
    debugMode: false,
    locationFields: ['job_city', 'job_state', 'location']
  });

  console.log(`\n🇺🇸 Filtered to ${usJobs.length} US-based jobs`);
  console.log(usJobs);
  return usJobs;
}
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node hpeScraper.js hardware engineering');
      console.log('Usage: node hpeScraper.js software');
      console.log('Usage: node hpeScraper.js "data science"');
      console.log('Or with quotes: node hpeScraper.js "machine learning"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping HPE Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await hpeScraper(searchQuery, 2);
...
//both
//have to do something with broadcom and hpe also intelbecause of the location id changes 