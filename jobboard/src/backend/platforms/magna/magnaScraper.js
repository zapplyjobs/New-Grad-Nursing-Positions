...
async function magnaScraper(searchQuery, maxPages = 10) {
  ...
  const allJobs = [];
  const baseUrl = 'https://wd3.myworkdaysite.com';
  ...
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const searchUrl = `${baseUrl}/en-US/recruiting/magna/Magna?q=${encodeURIComponent(searchQuery)}&Country=bc33aa3152ec42d4995f4791a106ed09`;
  ...
  console.log(`\n magna Scraping completed! Found ${allJobs.length} total jobs.`);
  return allJobs;
}
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node magnaScraper.js "hardware engineering"');
      console.log('Usage: node magnaScraper.js software');
      console.log('Usage: node magnaScraper.js "data science"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping magna Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await magnaScraper(searchQuery, 2);
