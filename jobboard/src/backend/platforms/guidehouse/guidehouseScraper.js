...
async function guidehouseScraper(searchQuery, maxPages = 10) {
  ...
  const allJobs = [];
  const baseUrl = 'https://guidehouse.wd1.myworkdayjobs.com';
  ...
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const searchUrl = `${baseUrl}/en-US/External?q=${encodeURIComponent(searchQuery)}&Location_Country=bc33aa3152ec42d4995f4791a106ed09`;
  ...
  console.log(`\n guidehouse Scraping completed! Found ${allJobs.length} total jobs.`);
  return allJobs;
}
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node guidehouseScraper.js "hardware engineering"');
      console.log('Usage: node guidehouseScraper.js software');
      console.log('Usage: node guidehouseScraper.js "data science"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping guidehouse Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await guidehouseScraper(searchQuery, 2);
...
module.exports = guidehouseScraper;
