...
async function boozallenScraper(searchQuery, maxPages = 30) {
  ...
  const allJobs = [];
  const baseUrl = 'https://bah.wd1.myworkdayjobs.com';
  
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      console.log(`Scraping page ${pageNum}...`);
      // Construct URL with search query
      const searchUrl = `${baseUrl}/en-US/BAH_Jobs?q=${encodeURIComponent(searchQuery)}`;
  ...
  console.log(`\n Booz ALLEN Scraping completed! Found ${allJobs.length} total jobs.`);
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node boozallenScraper;.js hardware engineering');
      console.log('Usage: node boozallenScraper;.js software');
      console.log('Usage: node boozallenScraper;.js "data science"');
      console.log('Or with quotes: node boozallenScraper;.js "machine learning"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping Booz ALLEN Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await boozallenScraper(searchQuery, 2);
...
module.exports = boozallenScraper;
