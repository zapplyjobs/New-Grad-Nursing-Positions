...
async function workdayScraper(searchQuery, maxPages = 10) {
  ...
  const allJobs = [];
  const baseUrl = 'https://workday.wd5.myworkdayjobs.com';
  ...
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const searchUrl = `${baseUrl}/Workday?q=${encodeURIComponent(searchQuery)}&Location_Country=bc33aa3152ec42d4995f4791a106ed09`;
        ...
        try {
          const jobData = await jobItem.evaluate((item) => {
            ...
            if (ddElements.length >= 2) {
              location = ddElements[1] ? ddElements[1].textContent.trim() : '';
              posted = ddElements[2] ? ddElements[2].textContent.trim() : '';
  ...
  console.log(`\n workday Scraping completed! Found ${allJobs.length} total jobs.`);
  return allJobs;
}
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node workdayScraper.js "hardware engineering"');
      console.log('Usage: node workdayScraper.js software');
      console.log('Usage: node workdayScraper.js "data science"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping Workday Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await workdayScraper(searchQuery, 2);
    ...
    if (jobs.length > 0) {
      ...
      // Show JSON format for first 2 jobs
