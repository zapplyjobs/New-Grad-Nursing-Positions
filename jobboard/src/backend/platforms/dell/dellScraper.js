...
async function DELLScraper(searchQuery, maxPages = 10) {
  ...
  const allJobs = [];
  const baseUrl = 'https://dell.wd1.myworkdayjobs.com';
  ...
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      const searchUrl = `${baseUrl}/External?q=${encodeURIComponent(searchQuery)}&Location_Country=bc33aa3152ec42d4995f4791a106ed09`;
  ...
  console.log(`\n DELL Scraping completed! Found ${allJobs.length} total jobs.`);
  return allJobs;
}
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node DELLScraper.js "hardware engineering"');
      console.log('Usage: node DELLScraper.js software');
      console.log('Usage: node DELLScraper.js "data science"');
      console.log('Note: maxPages parameter controls number of pages to scrape');
      return;
    }
    
    console.log(`=== Scraping DELL Careers for: "${searchQuery}" (10 pages max) ===`);
    const jobs = await DELLScraper(searchQuery, 2);
