...
async function gditScraper(searchQuery, maxPages = 30) {
  ...
  const allJobs = [];
  const baseUrl = 'https://gdit.wd5.myworkdayjobs.com';
  
  try {
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      console.log(`Scraping page ${pageNum}...`);
      // Construct URL with search query
      const searchUrl = `${baseUrl}/External_Career_Site?q=${encodeURIComponent(searchQuery)}`;
...
async function main() {
  try {
    ...
    if (!searchQuery) {
      ...
      console.log('Usage: node gditScraper;.js hardware engineering');
      console.log('Usage: node gditScraper;.js software');
      console.log('Usage: node gditScraper;.js "data science"');
      console.log('Or with quotes: node gditScraper;.js "machine learning"');
    ...
    const jobs = await gditScraper(searchQuery, 2);
